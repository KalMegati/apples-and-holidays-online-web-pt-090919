...
def add_supply_to_winter_holidays(holiday_hash, supply)
  ...
  holiday_hash[:winter].each { |party, favors| favors << supply }
...
def all_winter_holiday_supplies(holiday_hash)
  ...
  holiday_hash[:winter].values.reduce { |list, supply| list.concat(supply) }
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.each { |season, celebration|
    celebration.each { |holiday, supply| 
      cookouts << holiday if supply.include?("BBQ")
    }
  }
  cookouts
end